...
## Introduction
The [second tutorial]({{< ref "docs/tutorials/microservices-2-helm-integration" >}}) in this series demonstrated how
...
add 2 [targets]({{< ref "reference/kluctl-project/targets" >}}) called `test` and `prod`. To do so, modify the
...
We've also introduced [`args`]({{< ref "reference/kluctl-project/targets#args" >}}) for each target, with each target
...
There are a few [predefined variables]({{< ref "reference/templating/predefined-variables" >}}) which are always available
...
preferred solution is use [`overrideNamespace`]({{< ref "reference/deployments/deployment-yml#overridenamespace" >}})
...
[`commonLabels`]({{< ref "reference/deployments/deployment-yml#commonlabels" >}}) in your root `deployment.yml` has
...
To do so, let's introduce [variables files]({{< ref "reference/templating/variable-sources" >}}) that contain
